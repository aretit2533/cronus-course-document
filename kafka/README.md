# Apache Kafka Course

Welcome to the comprehensive Apache Kafka course! This course will take you from Kafka fundamentals to advanced concepts, with hands-on exercises and real-world applications.

## 📑 Table of Contents

- [🎯 Course Overview](#-course-overview)
- [👥 Target Audience](#-target-audience)
- [📋 Prerequisites](#-prerequisites)
- [📚 Course Modules](#-course-modules)
  - [Module 1: Introduction to Apache Kafka](#module-1-introduction-to-apache-kafka) | [📖 Content](module-01-introduction.md) | [💡 Exercises](exercise/module-01-exercises.md)
  - [Module 2: Kafka Architecture and Core Concepts](#module-2-kafka-architecture-and-core-concepts) | [📖 Content](module-02-architecture.md) | [💡 Exercises](exercise/module-02-exercises.md)
  - [Module 3: Setting Up Kafka](#module-3-setting-up-kafka) | [📖 Content](module-03-setup.md) | [💡 Exercises](exercise/module-03-exercises.md)
  - [Module 4: Kafka Producers](#module-4-kafka-producers) | [📖 Content](module-04-producers.md) | [💡 Exercises](exercise/module-04-exercises.md)
  - [Module 5: Kafka Consumers](#module-5-kafka-consumers) | [📖 Content](module-05-consumers.md) | [💡 Exercises](exercise/module-05-exercises.md)
  - [Module 6: Topics, Partitions, and Data Management](#module-6-topics-partitions-and-data-management) | [📖 Content](module-06-data-management.md) | [💡 Exercises](exercise/module-06-exercises.md)
  - [Module 7: Kafka Connect and Kafka Streams](#module-7-kafka-connect-and-kafka-streams) | [📖 Content](module-07-connect-streams.md) | [💡 Exercises](exercise/module-07-exercises.md)
  - [Module 8: Advanced Topics and Best Practices](#module-8-advanced-topics-and-best-practices) | [📖 Content](module-08-advanced.md) | [💡 Exercises](exercise/module-08-exercises.md)
  - [Module 9: Practical Application - Building a Real-Time System](#module-9-practical-application---building-a-real-time-system) | [📖 Content](module-09-practical.md) | [💡 Exercises](exercise/module-09-exercises.md)
- [💡 All Exercises](exercise/README.md)
- [📋 Course Outline](course-outline.md)
- [🛠️ Tools and Technologies](#️-tools-and-technologies)
- [⏱️ Course Duration](#️-course-duration)
- [📊 Learning Path](#-learning-path)
- [🎓 What You'll Learn](#-what-youll-learn)
- [📖 How to Use This Course](#-how-to-use-this-course)
- [🔗 Additional Resources](#-additional-resources)
- [🎯 Next Steps After Completion](#-next-steps-after-completion)
- [📝 Assessment](#-assessment)
- [🎖️ Course Completion](#️-course-completion)
- [📧 Support and Community](#-support-and-community)

---

## 🚀 Quick Start

| Module | Content | Exercises | Duration |
|--------|---------|-----------|----------|
| **Module 1** | [Introduction to Apache Kafka](module-01-introduction.md) | [Exercises](exercise/module-01-exercises.md) | 2 hours |
| **Module 2** | [Kafka Architecture and Core Concepts](module-02-architecture.md) | [Exercises](exercise/module-02-exercises.md) | 3 hours |
| **Module 3** | [Setting Up Kafka](module-03-setup.md) | [Exercises](exercise/module-03-exercises.md) | 2 hours |
| **Module 4** | [Kafka Producers](module-04-producers.md) | [Exercises](exercise/module-04-exercises.md) | 3 hours |
| **Module 5** | [Kafka Consumers](module-05-consumers.md) | [Exercises](exercise/module-05-exercises.md) | 3 hours |
| **Module 6** | [Topics, Partitions, and Data Management](module-06-data-management.md) | [Exercises](exercise/module-06-exercises.md) | 2 hours |
| **Module 7** | [Kafka Connect and Kafka Streams](module-07-connect-streams.md) | [Exercises](exercise/module-07-exercises.md) | 4 hours |
| **Module 8** | [Advanced Topics and Best Practices](module-08-advanced.md) | [Exercises](exercise/module-08-exercises.md) | 3 hours |
| **Module 9** | [Practical Application - Building a Real-Time System](module-09-practical.md) | [Exercises](exercise/module-09-exercises.md) | 4 hours |

**📂 [View All Exercises](exercise/README.md)** | **📋 [Detailed Course Outline](course-outline.md)**

## 🎯 Course Overview

Apache Kafka is a distributed event streaming platform used by thousands of companies for high-performance data pipelines, streaming analytics, data integration, and mission-critical applications. This course covers everything you need to become proficient with Kafka.

## 👥 Target Audience

- Backend Developers
- Data Engineers
- Software Architects
- DevOps Engineers
- Anyone interested in event-driven architectures and real-time data processing

## 📋 Prerequisites

- Basic understanding of distributed systems
- Familiarity with command-line tools
- Programming experience (Java, Python, or Node.js recommended)
- Understanding of basic networking concepts

## 📚 Course Modules

### [💡 Hands-On Exercises](exercise/README.md)

### Module 1: Introduction to Apache Kafka
**Duration:** 2 hours

Learn the fundamentals of Apache Kafka, event streaming, and when to use Kafka in your architecture.

**[📖 Module Content](module-01-introduction.md)**
**[💡 Exercises](exercise/module-01-exercises.md)**

---

### Module 2: Kafka Architecture and Core Concepts
**Duration:** 3 hours

Deep dive into Kafka's distributed architecture, including brokers, topics, partitions, and replication.

**[📖 Module Content](module-02-architecture.md)**
**[💡 Exercises](exercise/module-02-exercises.md)**

---

### Module 3: Setting Up Kafka
**Duration:** 2 hours

Learn how to install and configure Kafka locally, with Docker, and explore essential CLI tools.

**[📖 Module Content](module-03-setup.md)**
**[💡 Exercises](exercise/module-03-exercises.md)**

---

### Module 4: Kafka Producers
**Duration:** 3 hours

Master Kafka producers, including serialization, partitioning strategies, and reliability configurations.

**[📖 Module Content](module-04-producers.md)**
**[💡 Exercises](exercise/module-04-exercises.md)**

---

### Module 5: Kafka Consumers
**Duration:** 3 hours

Build robust Kafka consumers with proper offset management, consumer groups, and error handling.

**[📖 Module Content](module-05-consumers.md)**
**[💡 Exercises](exercise/module-05-exercises.md)**

---

### Module 6: Topics, Partitions, and Data Management
**Duration:** 2 hours

Design and manage Kafka topics effectively with proper retention, compaction, and schema management.

**[📖 Module Content](module-06-data-management.md)**
**[💡 Exercises](exercise/module-06-exercises.md)**

---

### Module 7: Kafka Connect and Kafka Streams
**Duration:** 4 hours

Integrate external systems with Kafka Connect and build stream processing applications with Kafka Streams.

**[📖 Module Content](module-07-connect-streams.md)**
**[💡 Exercises](exercise/module-07-exercises.md)**

---

### Module 8: Advanced Topics and Best Practices
**Duration:** 3 hours

Secure, monitor, and optimize Kafka clusters for production environments.

**[📖 Module Content](module-08-advanced.md)**
**[💡 Exercises](exercise/module-08-exercises.md)**

---

### Module 9: Practical Application - Building a Real-Time System
**Duration:** 4 hours

Build a complete event-driven system integrating all concepts learned throughout the course.

**[📖 Module Content](module-09-practical.md)**
**[💡 Exercises](exercise/module-09-exercises.md)**

---

## 🛠️ Tools and Technologies

- **Apache Kafka 3.x** - Core streaming platform
- **Docker and Docker Compose** - Container orchestration
- **Programming Languages:** Java 11+, Python 3.8+, Node.js 16+
- **UI Tools:** Kafka UI, Conduktor, Offset Explorer
- **Monitoring:** Prometheus, Grafana
- **Schema Registry** - Schema management
- **Kafka Connect** - Data integration

## ⏱️ Course Duration

**Total:** 26 hours
- Theory: 14 hours
- Hands-on Labs: 12 hours

## 📊 Learning Path

```
Introduction → Architecture → Setup → Producers → Consumers
     ↓
Data Management → Connect & Streams → Advanced Topics → Real Project
```

## 🎓 What You'll Learn

- ✅ Understand event streaming and event-driven architecture
- ✅ Set up and configure Kafka clusters
- ✅ Build reliable producers and consumers
- ✅ Implement stream processing applications
- ✅ Integrate Kafka with external systems
- ✅ Secure and monitor production Kafka clusters
- ✅ Apply best practices for performance and reliability
- ✅ Build real-world event-driven applications

## 📖 How to Use This Course

1. **Follow the modules in order** - Each module builds on previous concepts
2. **Complete the exercises** - Hands-on practice is essential for mastery
3. **Build the final project** - Apply all concepts in Module 9
4. **Refer to official documentation** - Kafka docs are comprehensive and well-maintained
5. **Practice regularly** - Set up local environments and experiment

## 🔗 Additional Resources

- [Apache Kafka Official Documentation](https://kafka.apache.org/documentation/)
- [Confluent Documentation](https://docs.confluent.io/)
- [Kafka Improvement Proposals (KIPs)](https://cwiki.apache.org/confluence/display/KAFKA/Kafka+Improvement+Proposals)
- [Kafka GitHub Repository](https://github.com/apache/kafka)
- [Confluent Blog](https://www.confluent.io/blog/)
- [Kafka Summit Videos](https://www.kafka-summit.org/)

## 🎯 Next Steps After Completion

- Explore **Confluent Cloud** and managed Kafka services
- Contribute to **open-source Kafka projects**
- Join **Kafka community** and attend meetups
- Prepare for **Confluent Certified Developer** certification
- Build **production-grade streaming applications**
- Study advanced patterns like **CQRS** and **Event Sourcing**

## 📝 Assessment

Each module includes:
- ✅ Conceptual review questions
- ✅ Hands-on coding exercises
- ✅ Real-world scenarios
- ✅ Final project in Module 9

## 🎖️ Course Completion

Upon completing all modules and exercises, you will:
- Have hands-on experience with Apache Kafka
- Understand production-ready patterns and practices
- Be able to build event-driven applications
- Be prepared for Kafka certification exams

## 📧 Support and Community

- Review the [course outline](course-outline.md) for detailed module breakdown
- Check [exercises](exercise/README.md) for hands-on practice
- Refer to official Kafka documentation for in-depth information
- Join Kafka community forums and Slack channels

---

**Ready to get started?** Begin with [Module 1: Introduction to Apache Kafka](module-01-introduction.md)!

🚀 Happy Learning!
